...
class JSONLoader(FileDataLoader):
    ...
    def _load_preprocessed_file(self):
        ...
        if not os.path.isdir(self.processed_data_dir):
            return False
        word_npy_file_name = os.path.join(self.processed_data_dir, name_prefix + '_word.npy')
        pos1_npy_file_name = os.path.join(self.processed_data_dir, name_prefix + '_pos1.npy')
        pos2_npy_file_name = os.path.join(self.processed_data_dir, name_prefix + '_pos2.npy')
        rel_npy_file_name = os.path.join(self.processed_data_dir, name_prefix + '_rel.npy')
        mask_npy_file_name = os.path.join(self.processed_data_dir, name_prefix + '_mask.npy')
        length_npy_file_name = os.path.join(self.processed_data_dir, name_prefix + '_length.npy')
        entpair2scope_file_name = os.path.join(self.processed_data_dir, name_prefix + '_entpair2scope.json')
        relfact2scope_file_name = os.path.join(self.processed_data_dir, name_prefix + '_relfact2scope.json')
        word_vec_mat_file_name = os.path.join(self.processed_data_dir, word_vec_name_prefix + '_mat.npy')
        word2id_file_name = os.path.join(self.processed_data_dir, word_vec_name_prefix + '_word2id.json')
    ...
    def __init__(self, dataset_dir, file_name, word_vec_file_name, rel2id_file_name, mode, shuffle=True, max_length=120, case_sensitive=False, reprocess=False, batch_size=160):
        '''
        ...
        self.processed_data_dir = os.path.join(dataset_dir, '_processed_data')
        ...
        if reprocess or not self._load_preprocessed_file(): # Try to load pre-processed files:
            ...
            np.save(os.path.join(self.processed_data_dir, name_prefix + '_word.npy'), self.data_word)
            np.save(os.path.join(self.processed_data_dir, name_prefix + '_pos1.npy'), self.data_pos1)
            np.save(os.path.join(self.processed_data_dir, name_prefix + '_pos2.npy'), self.data_pos2)
            np.save(os.path.join(self.processed_data_dir, name_prefix + '_rel.npy'), self.data_rel)
            np.save(os.path.join(self.processed_data_dir, name_prefix + '_mask.npy'), self.data_mask)
            np.save(os.path.join(self.processed_data_dir, name_prefix + '_length.npy'), self.data_length)
            json.dump(self.entpair2scope, open(os.path.join(self.processed_data_dir, name_prefix + '_entpair2scope.json'), 'w'))
            json.dump(self.relfact2scope, open(os.path.join(self.processed_data_dir, name_prefix + '_relfact2scope.json'), 'w'))
            np.save(os.path.join(self.processed_data_dir, word_vec_name_prefix + '_mat.npy'), self.word_vec_mat)
            json.dump(self.word2id, open(os.path.join(self.processed_data_dir, word_vec_name_prefix + '_word2id.json'), 'w'))
    ...
    def next_batch(self, batch_size):
        ...
        if self.mode == self.MODE_INSTANCE:
            ...
            for i in range(idx0, idx1):
                # order keeps tot num of relation-entities/entity-pairs/entities depending on mode
